...
void main() {
    ...
    // Flip, center, and scale the icon texture
    vec2 st = vec2(vUvs.x, 1.0 - vUvs.y); // Flip Y to ensure icons are upstraight
    st = (st - 0.5) * uIconScale + 0.5;    // Scale from the center

    // Discard pixels outside the scaled area to prevent texture bleeding
    if (st.x < 0.0 || st.x > 1.0 || st.y < 0.0 || st.y > 1.0) {
        discard;
    }

    vec2 finalUvs = st * cellSize + cellOffset;
    
    outColor = texture(uTex, finalUvs);
    outColor.a *= vAlpha;
}
`;
...
class InfiniteGridMenu {
  ...
  private discLocations!: {
    ...
    uIconScale: WebGLUniformLocation | null;
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discLocations = {
      ...
      uIconScale: gl.getUniformLocation(this.discProgram!, "uIconScale"),
        ...
        (item) =>
          new Promise<HTMLImageElement>((resolve) => {
            ...
            img.onerror = () => {
              ...
              fallbackCtx.fillStyle = '#3b82f6'; // Fallback color
  ...
  private render(): void {
    ...
    gl.uniform1f(this.discLocations.uIconScale, 1.4); // Scale factor for icons ( > 1 = smaller)
